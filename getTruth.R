...
# get truth for sine wave scenario
adherant_patient.sine <- function(id,L1,U,Era,DAYSUPP,PRICE,FOLLOWUP){
...
getTruth.sine <- function(N,DAYSUPP,PRICE,FOLLOWUP){
  brand   <- data.frame(id=NA,day=NA,S=NA)
  generic <- data.frame(id=NA,day=NA,S=NA)
  ...
  # create adherant patients timeline
  for(i in 1:N){
    brand <- rbind(brand,adherant_patient.sine(id=i,L1=L1b[i],U=u_star,Era=0,
                                          DAYSUPP=DAYSUPP,PRICE=PRICE,FOLLOWUP=FOLLOWUP))
    generic <- rbind(generic,adherant_patient.sine(id=i,L1=L1g[i],U=u_star,Era=1,
                                              DAYSUPP=DAYSUPP,PRICE=PRICE,FOLLOWUP=FOLLOWUP))
  }
