...
g_comp <- function(BSLN,MODELS,DAYSUPP,PRICE,FOLLOWUP){
  n <- BSLN[,.N]
...
analyze <- function(DATA,BAND,NUMSIM,DAYSUPP,PRICE,FOLLOWUP){
  ...
  brand   <- g_comp(bsln[U < u_star],b_mods, DAYSUPP=DAYSUPP, PRICE=PRICE, FOLLOWUP=FOLLOWUP)
  generic <- g_comp(bsln[U >= u_star],g_mods, DAYSUPP=DAYSUPP, PRICE=PRICE, FOLLOWUP=FOLLOWUP)
